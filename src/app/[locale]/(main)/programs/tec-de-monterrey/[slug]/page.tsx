...
export default async function ProgramDetailPage(props: { params: Promise<{ slug: string }> }) {
    ...
    let program = tecPrograms.find((p) => p.slug === params.slug);
    ...
    try {
        // Use ID for robust linking as requested
        const dynamicProgram = await ProgramService.getById(program.id);
        if (dynamicProgram) {
            console.log(`[Pricing] Loaded dynamic pricing for ${program.id}: $${dynamicProgram.price}`);
            program = {
                ...program,
                price: dynamicProgram.price,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                currency: (dynamicProgram.currency as any) || program.currency,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                plans: (dynamicProgram.plans as any[]) || program.plans,
            };
        }
    } catch (error) {
        console.warn(`[Pricing] Failed to load dynamic pricing for ${program.id}`, error);
    }
        ...
        <main className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="bg-slate-900 text-white py-4">
                <div className="container-custom">
                    <Link href="/programs/tec-de-monterrey" className="inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white transition-colors">
